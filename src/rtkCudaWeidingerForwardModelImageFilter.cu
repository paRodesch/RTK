  ...
  for (unsigned int r=0; r<nEnergies; r++)
    for (unsigned int c=0; c<nMaterials; c++)
      for (unsigned int c2=0; c2<nMaterials; c2++)
        ...
        // Fessler Surrogate :
        // if (attenuationFactors[r]<0.05)
        //   {
        //       intermForHessian[(r * nMaterials + c) * nMaterials + c2] = c_materialAttenuations[c + nMaterials * r] * c_materialAttenuations[c2 + nMaterials * r];
        //   }
        //   else
        //   {
        //       intermForHessian[(r * nMaterials + c) * nMaterials + c2] = 1.1*c_materialAttenuations[c + nMaterials * r] * c_materialAttenuations[c2 + nMaterials * r] * (1-expAttenuationFactors[r]-attenuationFactors[r]*expAttenuationFactors[r])/attenuationFactors[r]/attenuationFactors[r];
        //   }
